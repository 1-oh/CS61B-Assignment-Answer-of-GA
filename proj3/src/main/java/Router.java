        ...
        for(int i = 0 ; i < route.size() - 1; i += 1){
           ...
           //Initialize the ways
            ...
            if(i == 0){
                ...
                ret.get(ret.size() - 1).distance += g.distance(PreNode, CurNode);
                if(g.EdgeMap.get(PreWay).tags.containsKey("name")){
                    ret.get(ret.size() -1).way = g.EdgeMap.get(PreWay).tags.get("name");
                }
                ...
                if(i == route.size() - 2) {
                    ret.get(ret.size() - 1).distance += g.distance(CurNode, NextNode);
                }
            }
