...
public class SeamCarver {
    ...
    public  double energy(int x, int y) {
        long upRGB = thePicture.getRGB(x, rowToUp(y));
        long downRGB = thePicture.getRGB(x, rowToDown(y));
        long leftRGB = thePicture.getRGB(colToLeft(x), y);
        long rightRGB = thePicture.getRGB(colToRight(x), y);

        double ret = 0;

        for (int i = 0; i < 3 ;i += 1){
            double difference = ((upRGB >> (8 * i)) & 0xFF) - ((downRGB >> (8 * i)) & 0xFF);
            ret += pow(difference, 2);
        }

        for (int i = 0; i < 3 ;i += 1){
            double difference = ((leftRGB >> (8 * i)) & 0xFF) - ((rightRGB >> (8 * i)) & 0xFF);
            ret += pow(difference, 2);
        }
