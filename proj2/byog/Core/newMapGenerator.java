...
public class newMapGenerator implements Serializable {
    ...
    public static int[][] isFull; /*0 for nothing,1 for floors and 2 for walls,3 for frog,4 for bear,5 for communism*/
    ...
    private void readObject(ObjectInputStream in) throws IOException, ClassNotFoundException {
        ...
        for(int i=0;i<width;i+=1){
            for(int j=0;j<height;j+=1){
                switch (isFull[i][j]){
                    case 0:Tiles[i][j]=Tileset.NOTHING;break;
                    case 1:Tiles[i][j]=Tileset.FLOOR;break;
                    case 2:Tiles[i][j]=Tileset.WALL;break;
                    case 3:Tiles[i][j]=Tileset.FROG;break;
                    case 4:Tiles[i][j]=Tileset.BEAR;break;
                    case 5:Tiles[i][j]=Tileset.COMMUNISM;
                }
            }
        }
...
/*-------------------------The part for Serializable(Saving the current situation of the class(End)-------------------*/
    ...
    private static boolean insert(Position p,TETile t){
        ...
        if(p.px<margin||p.px>=width-margin||p.py<margin||p.py>=height-margin) return false;
        if(isAnything(p)){
            return false;
        }
        Tiles[p.px][p.py]=t;
    ...
    public static void MapGenerate(){
        ...
        Tiles[Frog.x][Frog.y]=Tileset.FROG;
        ...
        Tiles[Bear.x][Bear.y]=Tileset.BEAR;
