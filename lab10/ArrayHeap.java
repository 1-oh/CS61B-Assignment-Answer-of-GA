...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    private void sink(int index) {
        ...
        /** TODO: Your code here. */
        int NodeToExChange = min(leftIndex(index) ,rightIndex(index));
        if(min(NodeToExChange, index) == index)
            return;
        else{
            swap(NodeToExChange, index);
            sink(NodeToExChange);
        }
    }
    ...
    /**
     * Returns the Node's item with the smallest priority value, but does not remove it
     ...
     */
    public int TraversalHelper(T item, int start){
        if(getNode(start) == null) return -1;
        if(getNode(start).myItem.equals(item)) return start;
        else{
            int ret = TraversalHelper(item, leftIndex(start));
            if(ret != -1) return ret;
            else return TraversalHelper(item, rightIndex(start));
        }
    }

    public int Traversal(T item){
        return TraversalHelper(item, 1);
    }

    @Override
    public void changePriority(T item, double priority) {
        /* TODO: Your code here! */
        int target=Traversal(item);
        if(target == -1) throw new RuntimeException();

        if(priority < contents[target].myPriority){
            contents[target].myPriority = priority;
            swim(target);
        }
        else if(priority > contents[target].myPriority){
            contents[target].myPriority = priority;
            sink(target);
        }
    }
